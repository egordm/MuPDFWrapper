...
public class DocActivityView extends FrameLayout implements TabHost.OnTabChangeListener, View.OnClickListener, DocView.SelectionChangeListener {
	...
	*/
	...
	protected void showPages() {
		LinearLayout pages = findViewById(R.id.pages_container);
		if (null == pages)
			return;
		...
		observer.addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
			...
			public void onGlobalLayout() {
				if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
					...
					mDocView.getViewTreeObserver().removeGlobalOnLayoutListener(this);
				}
	...
	public void afterFirstLayoutComplete(final String path) {
		//  main view
		mDocView = findViewById(R.id.doc_view_inner);
		mDocView.setHost(this);
		mDocReflowView = findViewById(R.id.doc_reflow_view);
		...
		if (usePagesView()) {
			...
			LinearLayout layout2 = findViewById(R.id.pages_container);
		...
		mReflowButton = findViewById(R.id.reflow_button);
		...
		showSearchSelected(false);
		mSearchText = findViewById(R.id.search_text_input);
		mSearchText.setOnClickListener(this);
		
		mSaveButton = findViewById(R.id.save_button);
		mSaveButton.setOnClickListener(this);
		
		mSaveAsButton = findViewById(R.id.save_as_button);
		...
		mOpenInButton = findViewById(R.id.open_in_button);
		...
		mSearchNextButton = findViewById(R.id.search_next_button);
		mSearchNextButton.setOnClickListener(this);
		
		mSearchPreviousButton = findViewById(R.id.search_previous_button);
		mSearchPreviousButton.setOnClickListener(this);
		
		mToggleAnnotButton = findViewById(R.id.show_annot_button);
		mToggleAnnotButton.setOnClickListener(this);
		
		mHighlightButton = findViewById(R.id.highlight_button);
		...
		mLineThicknessButton = findViewById(R.id.line_thickness_button);
	...
	private String getEmbeddedProfileName() {
		PDFDocument pdfDoc = (PDFDocument) mDoc;
		PDFObject outputIntents = pdfDoc.getTrailer().get("Root").get("OutputIntents");
	...
	private void onProof() {
		...
		final Spinner sp1 = dialog.findViewById(R.id.print_profile_spinner);
		final Spinner sp2 = dialog.findViewById(R.id.display_profile_spinner);
		final Spinner sp3 = dialog.findViewById(R.id.resolution_spinner);
	...
	public void onSelectionChanged() {
		...
		if (note != lastNote) {
			TextView tv = findViewById(R.id.doc_note_editor_date);
			EditText te = findViewById(R.id.doc_note_editor_text);

...
import com.artifex.mupdf.android.PageAdapter;
import com.artifex.mupdf.android.RenderListener;
import com.artifex.mupdf.android.Stepper;
import com.artifex.mupdf.fitz.Document;

/**
 * Created by egordm on 14-8-2017.
 */

public class PDFViewBase extends AdapterView<Adapter>
		implements GestureDetector.OnGestureListener, ScaleGestureDetector.OnScaleGestureListener, Runnable {
	
	interface PDFViewPageObserver {
		void setCurrentPDFPage(int page);
	}
	...
	public PDFViewBase(Context context) {
		super(context);
		initialize(context);
	}
	
	public PDFViewBase(Context context, AttributeSet attrs) {
		super(context, attrs);
		initialize(context);
	}
	
	public PDFViewBase(Context context, AttributeSet attrs, int defStyle) {
	...
	protected PDFViewPageObserver mObserver = null;
	
	public void setPageObserver(PDFViewPageObserver observer) {
		mObserver = observer;
	}
	...
	protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
		...
		for (int i = 0; i < numDocPages; i++) {
			DocPageView cv = (DocPageView) getOrCreateChild(i); // TODO Height is 0 when not fully loaded. Mb another approach
